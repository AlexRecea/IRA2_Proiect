...
G0 = evalfr(G, 0); %calculul pt s=0 -> calculam gain
G0_inv = inv(G0);
K_static = G0_inv;
G_Decoupled_static = K_static * G; %sist decuplat in regim stationar
...
HR_azimuth_min = minreal (zpk (HR_azimuth)); %regulator bun! :D
...
HR_pitch_min = minreal (zpk (HR_pitch)); %ne bucuram din nou pentru :P
...
sys = minreal(zpk (sys_closed_loop))
lsim (sys, [azimuth_input, pitch_input], time)
